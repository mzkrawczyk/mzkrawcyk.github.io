...
// Load site data from JSON script tag
...
function loadLanguagePreference() {
    try {
        ...
        if (saved && ['EN', 'SV', 'PL'].includes(saved)) {
            currentLang = saved;
        }
    } catch (e) {
...
function t(key) {
    ...
    return value || siteData.translations.EN?.[key] || key;
}
            ...
            <section class="featured-video">
                <div class="video-container">
                    <div class="video-wrapper">
                        <iframe 
                            src="https://www.youtube.com/embed/Wzu14K46W-8" 
                                            ...
                                            <div class="card">
                                                ...
                                                <div class="card-actions">
                                                    ...
                                                    <button class="btn btn--secondary btn--sm" onclick="toggleMoreInfo(this)">
                                                        ${t('moreInfo')} <span class="chevron">▼</span>
                                                    </button>
                                                </div>
                                            </div>
                                        `).join('')}
...
function generateWorkButtons(work) {
    if (work.withdrawn) {
        return `<button class="btn btn--disabled btn--sm" disabled aria-disabled="true">${t('notForPerformance')}</button>`;
    }
    
    let buttons = '';
    
    if (work.score_link) {
        buttons += `<a href="${work.score_link}" target="_blank" class="btn btn--primary btn--sm">${t('viewScore')}</a>`;
    } else if (work.needs_contact) {
        buttons += `<a href="mailto:mkrawczykcomposer@hotmail.com?subject=${encodeURIComponent('Score request: ' + work.title)}" class="btn btn--primary btn--sm">${t('contactScore')}</a>`;
    }
    
    const recording = siteData.recordings[work.title];
    if (recording) {
        buttons += `<a href="${recording}" target="_blank" class="btn btn--secondary btn--sm">${t('listen')}</a>`;
    }
...
function renderContact() {
    const main = document.querySelector('.main-content');
    const displayEmail = 'mkrawczykcomposer [at] hotmail [dot] com';
    ...
    main.innerHTML = `
        <div class="view">
            <div class="container">
                ...
                <textarea id="email-fallback" style="position: absolute; left: -9999px;">mkrawczykcomposer@hotmail.com</textarea>
                
                <div class="social-links">
                    <a href="${siteData.recordings.soundcloud}" target="_blank" class="social-link">
                    ...
                    <a href="${siteData.recordings.youtube}" target="_blank" class="social-link">
                        <svg viewBox="0 0 24 24" fill="currentColor">
...
function copyEmail(button) {
    const email = 'mkrawczykcomposer@hotmail.com';
